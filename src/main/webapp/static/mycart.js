$(document).ready(
		function() {
			...
			$("#registerForm input[type=text][name='username']").blur(function(){
				checkusername($(this));
			});
...
function loginUser() {
	...
	hideSpan($('#loginMessageHolder > span.loader'));
	$('.loginMessage').html('');
			...
			.ajax(
					{
						...
						beforeSend : function() {
							...
							showSpan($('#loginMessageHolder > span.loader'));
						},
						success : function(responsedata) {
							$('#loginMessageHolder > span.loader').hide();
							if (responsedata.success) {
...
function checkusername($input) {
	$('#user_Name_Submit').attr("disabled",true);
	$input.parent().find('.error').remove();
	var valid = false;
	$.ajax({
		method : "GET",
		contentType : "application/json",
		url : "http://localhost:8093/api/customer/checkusername/"+$input.val(),
		timeout : 100000,
		beforeSend : function() {
			// setting a timeout
		},
		success : function(responsedata) {
			if (responsedata) {
				...
				$input.after('<span class="error">Username is not available</span>');
				$('#user_Name_Submit').attr("disabled",true);
			} else {
				//this is available
				$('#user_Name_Submit').removeAttr("disabled");
				valid=true;
			}
	...
	console.log('Validation for' + $input.val() +":---  "+valid);
	return valid;
}